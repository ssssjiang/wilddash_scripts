...
def path2fn(p, check_folder=False):
    ...
    p0 = os.path.splitext(os.path.basename(p))[0].lower().replace('_polygons', '').replace('_gtfine', '').replace(
        '_gtcoarse', '')
...
def downl_main(argv=sys.argv[1:]):
    ...
    errors, warnings, success_cnt = remap_inplace_json(change_path=args.change_path,
                                                       ...
                                                       tqdm_vers=tqdm_vers)
    if not args.silent:
        print("Finished delta remapping opertation with %i successes, %i warnings, and %i errors." % (
        success_cnt, len(warnings), len(errors)))
